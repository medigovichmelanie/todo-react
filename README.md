...
1. When is the functionality being triggered?: When the 'Add new task' button is clicked.
...
4. In the action, we dispatch a new ADD_NEW_TASK action.
5. In the store, we handle the action adding a new case into the SWITCH.
...
8. On all the dependent components that are listening to the store emmit, we add some logic on the emmit handler to react to that particular emmit and update their respective state objects if needed.
